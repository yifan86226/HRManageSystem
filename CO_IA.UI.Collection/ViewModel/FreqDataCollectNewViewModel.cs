...
{
    public class FreqDataCollectNewViewModel : ViewModelBase, IDisposable
    {
        public event Action<List<Data.Collection.AnalysisResult>, List<Data.Collection.AnalysisResult>, List<Data.Collection.AnalysisResult>,long> AfterMonitor;
        ...
        public MonitorPlanInfo FPS = null;
        /// <summary>
        /// 传递过来的装备集合
        /// </summary>
        ...
        //add by michael
        public List<ActivityEquipment> EquipmentList;
        public List<ActivityEquipment> EquipmentListMate = new List<ActivityEquipment>();
        public List<CO_IA_Data.ActivitySurroundStation> RoundStationInfoList;
        public List<CO_IA_Data.ActivitySurroundStation> RoundStationInfoListMate = new List<CO_IA_Data.ActivitySurroundStation>();
        //end
        ...
        public string MonitorPlanID = ""; //add by michael 17.07.20

        public FreqDataCollectNewViewModel()
        {
            ...
            try
            {
                ...
                OpenReciverCommand = new RelayCommand(() =>
                {
                    ...
                    {
                        if (RadioAVisibility == System.Windows.Visibility.Visible)
                        {
                            if (!Sensor.Connect(RoadTestInfor.ReciverIp))
                            {
                        ...
                        {
                            //add by michael 
                             dataHandler = RMTP_SEGMENT_CALLBACK1;
                             msgHandler = Msg_CALLBACK;
                             if (AgilentDll.Sensor.RMTPConnect(RoadTestInfor.ReciverIp, RoadTestInfor.Port, RoadTestInfor.Username, RoadTestInfor.Password, msgHandler, dataHandler) != 0)
                            {
                                Sensor.IsUseSensor = false;
                                Messenger.Default.Send<GenericMessage<string>>(new GenericMessage<string>(this, "ErrorConnectReciver"));
                                return;
                ...
                StopMeasurCommand = new RelayCommand(() =>
                {
                    ...
                    {
                        Sensor.RMTPSendStopWbqexCmd();
                        _CollectionDataSave.updateSqliteIndex();
                    }
                    //end
                ...
                //add by michael 2017.07.13
                RadioACommand = new RelayCommand(ExecuteRadioACommand,null);
                RadioBCommand = new RelayCommand(ExecuteRadioBCommand,null);
                //end
                #endregion
                ...
                _LineChartViewModel = new ChartNewViewModel(this);
        ...
        private ChartNewViewModel _LineChartViewModel;
        /// <summary>
        /// 图表控件实体
        /// </summary>
        public ChartNewViewModel LineChartViewModel
        {
            get
            {
        ...
        //add by michael 2017.07.13
        //private System.Windows.Visibility _RadioAVisibility = System.Windows.Visibility.Visible;
        private System.Windows.Visibility _RadioAVisibility = System.Windows.Visibility.Collapsed;
        public System.Windows.Visibility RadioAVisibility
        {
            get
            {

                return _RadioAVisibility;
            }
            set
            {
                Set(() => RadioAVisibility, ref _RadioAVisibility, value);
            }
        }
        //private System.Windows.Visibility _RadioBVisibility = System.Windows.Visibility.Collapsed;
        private System.Windows.Visibility _RadioBVisibility = System.Windows.Visibility.Visible;
        public System.Windows.Visibility RadioBVisibility
        {
            get
            {

                return _RadioBVisibility;
            }
            set
            {
                Set(() => RadioBVisibility, ref _RadioBVisibility, value);
            }
        }
        //end
        ...
        //add by michael
        public RelayCommand RadioACommand { get; private set; }
        public RelayCommand RadioBCommand { get; private set; }

        private AgilentDll.Sensor.RMTP_SEGMENT_CALLBACK dataHandler;
        private AgilentDll.Sensor.Msg_CALLBACK msgHandler;
        //private UInt32 FreqPointNum = 0;
        //private double FreqPointStart = 0;
        //end
        ...
        void ExecuteStartMeasurCommand()
        {
            ...
            if (Sensor.IsUseSensor)
            {
                ...
                //if (EntityQuery<RoadTest>.Instance.Insert(rt) > 0)
                {
                    ...
                    {
                        int status = AgilentDll.Sensor.RMTPConnect(RoadTestInfor.ReciverIp, RoadTestInfor.Port, RoadTestInfor.Username, RoadTestInfor.Password, msgHandler, dataHandler);
                        if (status == 0 || status == -10002)
                            status = AgilentDll.Sensor.RMTPSendWbqexCmd(RoadTestInfor.StartFreq, RoadTestInfor.EndFreq, RoadTestInfor.Bandwidth);                        
                    }
                }
        ...
        void ExecuteStartRealTimeMonitorCommand()
        {
            ...
            if (pingEquipment(RoadTestInfor.ReciverIp))
            {
                ...
                {
                    dataHandler = RMTP_SEGMENT_CALLBACK2;
                    msgHandler = Msg_CALLBACK;
                    if (AgilentDll.Sensor.RMTPConnect(RoadTestInfor.ReciverIp, RoadTestInfor.Port, RoadTestInfor.Username, RoadTestInfor.Password, msgHandler, dataHandler) !=0)
                    {
                        Sensor.IsUseSensor = false;
                    ...
                    else
                    {
                        ...
                        //TextColor = Brushes.Green;
                        //TextStatus = "连接状态：连接成功";
                        AgilentDll.Sensor.RMTPSendWbqexCmd(RoadTestInfor.StartFreq, RoadTestInfor.EndFreq, RoadTestInfor.Bandwidth);                        
                    }
                    //end
                }
            }
        ...
        //add by michael 2017.07.13
        private void ExecuteRadioACommand()
        {
            RadioAVisibility = System.Windows.Visibility.Visible;
            RadioBVisibility = System.Windows.Visibility.Collapsed;
        }

        private void ExecuteRadioBCommand()
        {
            RadioAVisibility = System.Windows.Visibility.Collapsed;
            RadioBVisibility = System.Windows.Visibility.Visible;
        }
        //end
        ...
        //add by michael 
        private int RMTP_SEGMENT_CALLBACK1(ref AgilentDll.Sensor.rmtpSegmentData dataHeader, IntPtr data)
        {
            //Console.WriteLine("{0}-{1}", dataHeader.startFrequency, dataHeader.numPoints);
            //if (dataHeader.startFrequency <= RoadTestInfor.StartFreq * 1000000.0)

            //Console.WriteLine("{0}-{1}", dataHeader.frameStartFrequency, dataHeader.frameNumPoints);

            if (dataHeader.frameStartFrequency <= RoadTestInfor.StartFreq * 1000000.0)
            {
                FreqMeasureId++;
                FreqMeasurePakageId = 0;
            }
            //FreqPointNum = dataHeader.numPoints;
            //FreqPointNum = dataHeader.frameNumPoints;

            FreqMeasurePakageId++;
            FreqDataIndex++;

            //Int16[] volList = new Int16[dataHeader.numPoints];
            //System.Runtime.InteropServices.Marshal.Copy(data, volList, 0, Convert.ToInt32(dataHeader.numPoints));
            Int16[] volList = new Int16[dataHeader.frameNumPoints];
            System.Runtime.InteropServices.Marshal.Copy(data, volList, 0, Convert.ToInt32(dataHeader.frameNumPoints));

            FreqLineDataItem FreqShowData = new FreqLineDataItem();
            //FreqShowData.byteArray = new float[dataHeader.numPoints];
            //for (int i = 0; i < dataHeader.numPoints; i++)
            FreqShowData.byteArray = new float[dataHeader.frameNumPoints];
            for (int i = 0; i < dataHeader.frameNumPoints; i++)
            {
                FreqShowData.byteArray[i] = (float)(volList[i] / 100.0 - 107);
            }
            FreqShowData.FreqMeasureId = FreqMeasureId;
            FreqShowData.FreqMeasurePakageId = FreqMeasurePakageId;
            FreqShowData.frequencyStep = dataHeader.frequencyStep;//maybe michael
            //FreqShowData.startFrequency = dataHeader.startFrequency;// detail.freq_start;//maybe michael            
            FreqShowData.startFrequency = dataHeader.frameStartFrequency;// detail.freq_start;//maybe michael
            FreqShowData.TestFreqStart = RoadTestInfor.StartFreq;
            FreqShowData.TestFreqEnd = RoadTestInfor.EndFreq;
            FreqShowData.DataIndex = FreqDataIndex;
        ...
        private int RMTP_SEGMENT_CALLBACK2(ref AgilentDll.Sensor.rmtpSegmentData dataHeader, IntPtr data)
        {
            //Console.WriteLine("{0}-{1}", dataHeader.startFrequency, dataHeader.numPoints);
            //Console.WriteLine("{0}-{1}-{2}-{3}", dataHeader.startFrequency, dataHeader.frameStartFrequency, RoadTestInfor.StartFreq, dataHeader.numPoints);
            ...
            {
                FreqShowData.byteArray[i] = (float)(volList[i] / 100.0 - 107);
                //FreqShowData.byteArray[i] = (float)(volList[i]/100);
            }
            FreqShowData.FreqMeasureId = FreqMeasureId;
            FreqShowData.FreqMeasurePakageId = FreqMeasurePakageId;
            FreqShowData.frequencyStep = dataHeader.frequencyStep;//maybe michael
        ...
        private int Msg_CALLBACK(ref AgilentDll.Sensor.rmtpMsgData msg)
        {
            //Console.WriteLine("{0}-{1}", msg.msgType, msg.MsgInfo);
            if (msg.MsgInfo.IndexOf("FAILURE;") > -1)
            {
                Messenger.Default.Send(msg.MsgInfo, "ShowMyMessage");
                AgilentDll.Sensor.RMTPSendStopWbqexCmd();
                AgilentDll.Sensor.IsUseSensor = false;
            }
            return 0;
        }
        //end
        ...
        private void dTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (FreqShowDataGrid == null)
                return;
            string strFpsKey = FPS.Key;
            string strActivityGuid = Activity.Guid;
            string strPlanId = MonitorPlanID;
            ...
            if (EquipmentList != null)
            {
                ...
                foreach (var tmp in tempList)
                {
                    ...
                    var equiment = EquipmentList.Find(obj => obj.AssignSendFreq == tmp.Frequency);//add by michael
                    ...
                    if (equiment != null)
                    {
                        ...
                        tmp.FreqGuid = equiment.Key;//add michael
                        //tmp.AmplitudeMaxValue = equiment.
                    ...
                    //List<CO_IA_Data.ActivitySurroundStation> roundStation = new List<CO_IA_Data.ActivitySurroundStation>();

                    //add by michael
                    CO_IA_Data.ActivitySurroundStation roundStation = null;

                    foreach (var station in RoundStationInfoList)
                    {
                        foreach (var equip in station.EmitInfo)
                        {
                            if (equip.FreqType == CO_IA_Data.FreqType.频段)
                            {
                                if (equip.FreqEFB <= tmp.Frequency && equip.FreqEFE >= tmp.Frequency)
                                {
                                    roundStation = station;
                                }
                            }
                            else
                            {
                                if (equip.FreqEC == tmp.Frequency)
                                {
                                    roundStation = station;
                                }
                            }
                ...
                foreach (var equment in EquipmentList)
                {
                    ...
                    if (!EquipmentListMate.Contains(equment) && equment.AssignSendFreq >= FreqShowDataGrid.TestFreqStart && equment.AssignSendFreq <= FreqShowDataGrid.TestFreqEnd)
                    {
                        AnalysisResult analysisResult = new AnalysisResult();
                        //analysisResult.FreqGuid = equment.GUID;
                        analysisResult.FreqGuid = equment.Key;
                        ...
                        analysisResult.Frequency = equment.AssignSendFreq != null ? (double)equment.AssignSendFreq : 0;//add by michael
                        analysisResult.BandWidth = RoadTestInfor.Bandwidth;
                        analysisResult.IsSend = false;
                        //analysisResult.AmplitudeMaxValue = Convert.ToInt32(kvp.Value[i]) + 107;
                        analysisResult.SendStatusPic = @"/CO_IA.UI.Collection;component/Images/wrong_24x24.png";
            ...
            backbroudWorker.DoWork += ((s, p) =>
            {
                ...
                PT_BS_Service.Client.Framework.BeOperationInvoker.Invoke<I_CO_IA_Collection, bool>(channel =>
                {
                    ...
                    channel.RemoveRealTimeMonitor(strFpsKey, strActivityGuid, strPlanId);//add by michael
                    ...
                    result0 = analysisList == null || analysisList.Count == 0 ? false : channel.UpLoadRealTimeMonitor(strFpsKey, strActivityGuid, strPlanId, analysisList.ToList(), '0');//add by michael
                    result1 = illegalAnalysisList == null || illegalAnalysisList.Count == 0 ? false : channel.UpLoadRealTimeMonitor(strFpsKey, strActivityGuid, strPlanId, illegalAnalysisList.ToList(), '1');
                    result1 = FreqShowDataGrid == null ? false : channel.UpLoadSpectrumData(strFpsKey, strActivityGuid, strPlanId, FreqShowDataGrid);
                    return result0 && result1 && result2;
                });
            });
            backbroudWorker.RunWorkerCompleted += ((s, p) =>
            {
                //System.Console.WriteLine("key=" + FPS.Key + ",hz=" + FPS.MHzFreqFrom + "," + FPS.MHzFreqTo);
                if (AfterMonitor != null)
                    AfterMonitor(analysisList.Where((obj) => { return obj.IsSend; }).ToList(), analysisList.Where((obj) => { return !obj.IsSend; }).ToList(), illegalAnalysisList.ToList(), totalTimes);
            });
