...
public class NPOIHelper
{
    ...
    /// <param name="dt"></param>
    /// <returns></returns>
    public static bool DataTableToExcelForRewaor(DataTable dt, string outputFile, int rowid, int coluid)
    {
        string oldFile = AppDomain.CurrentDomain.BaseDirectory + "\\统计\\加分情况统计.xlsx";


        if (File.Exists(oldFile))
        {

            File.Copy(oldFile, outputFile,true);

        }
        else
        {
            wl.WriteLogs("未找到原版本文件" + oldFile);
        ...
        //IWorkbook workbook = WorkbookFactory.Create(outputFile);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
                    ...
                    for (int j = 0; j < columnCount; j++)
                    {
                        ...
                        //excel第rowid +1行开始写入数据  
                        if (sheet.GetRow(i + rowid) == null)
                        {
                            sheet.CreateRow(i + rowid);
                        }
                        if (sheet.GetRow(i + rowid).GetCell(coluid) == null)
                        {
                            sheet.GetRow(i + rowid).CreateCell(coluid);
                        }

                        sheet.GetRow(i + rowid).GetCell(coluid).SetCellValue(dt.Rows[i].ItemArray[j].ToString());
                    }

                }
                using (fs =  new FileStream(outputFile, FileMode.OpenOrCreate, FileAccess.Write))
                {
    ...
    public static byte[] ExcelToImage(string filepath)
    {
        try
        {
            using (FileStream fsReader = File.OpenRead(filepath))
            {
                ...
                if (pictures.Count > 0)
                {
                    ...
                    return ((HSSFPictureData)pictures[0]).Data;
                }
                //int i = 0;
    ...
    public static void ExportByWeb(DataSet ds, string strHeaderText, string strFileName)
    {
        ...
        //else
        //{
        //    curContext.Response.AppendHeader("Content-Disposition", "attachment;filename=" + DXHttpUtility.UrlEncode(strFileName, System.Text.Encoding.UTF8));
        //}
...
{
    public void WriteLogs(string v)
    {
